...
export function EmailModal({ isOpen, onClose }) {
  ...
  const handleSubmit = (e) => {
    ...
    const emailSub = encodeURIComponent(`[CA Eco Hub] ${subject || 'Feedback'}`);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
      <div className="w-full max-w-lg rounded-2xl border border-emerald-500/40 bg-slate-900 p-6 space-y-4 shadow-2xl">
        <div className="flex justify-between items-center pb-2 border-b border-slate-800">
          <h3 className="text-lg font-bold text-white flex items-center gap-2">
            📧 Send Feedback / Message (Economics)
          </h3>
        ...
        {sentSuccess ? (
          <div className="p-4 rounded-xl bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 text-center font-bold text-sm">
            ✓ Opening email client... Economics feedback message ready!
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Economics Feedback, Question doubt..."
                className="w-full rounded-xl bg-slate-800 border border-slate-700 px-3.5 py-2 text-xs text-white focus:border-emerald-500 focus:outline-none"
              />
            </div>
            <div>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button type="submit" className="px-5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-xs font-bold text-white shadow-lg cursor-pointer">Send Email 🚀</button>
            </div>
          </form>
        )}
      </div>
