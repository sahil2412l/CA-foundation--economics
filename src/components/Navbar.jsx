...
export function Navbar({ user, activeTab, onOpenQuickWindow, onOpenEmailModal, onOpenNoticeModal, isSidebarCollapsed, onToggleSidebar }) {
  const tabTitles = {
    dashboard: 'Economics Dashboard Overview',
    chapters: 'CA Foundation Economics & BCK Syllabus',
    table: 'Economics Question Bank Data Table',
    flashcards: 'Economics & BCK Formula Flashcards',
    calculator: 'Economics & Elasticity Calculator',
  ...
  return (
    <header className="sticky top-0 z-40 flex h-20 items-center justify-between border-b border-slate-800 bg-slate-900/90 px-8 backdrop-blur-xl transition-all duration-300">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">{tabTitles[activeTab] || 'CA Foundation Economics'}</h1>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button
          ...
          title="View Economics Syllabus Notice"
        >
          <span>ℹ️ Notice</span>
        </button>
