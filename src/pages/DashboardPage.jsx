...
export function DashboardPage({ user, setActiveTab }) {
  ...
  return (
    <div className="mx-auto max-w-7xl p-8 space-y-8">
      <div className="rounded-2xl border border-emerald-500/30 bg-gradient-to-r from-emerald-950/80 via-slate-900/90 to-slate-950 p-8 shadow-xl backdrop-blur-xl">
        <div className="flex flex-wrap items-center justify-between gap-6">
          <div>
            <div className="inline-flex items-center gap-1.5 rounded-full border border-cyan-500/30 bg-cyan-500/10 px-3 py-1 text-xs font-semibold text-cyan-400 mb-3">
              🎯 Target: CA Foundation Business Economics & Macroeconomics
            </div>
            <h1 className="text-3xl font-extrabold text-white">
              Hello, <span className="bg-gradient-to-r from-emerald-300 via-cyan-300 to-indigo-300 bg-clip-text text-transparent">{user.name}</span>! 👋
            </h1>
            <p className="mt-2 text-slate-400 text-sm max-w-2xl">
              You are currently at <strong className="text-emerald-400">Level {user.level}</strong> with rank title{' '}
          ...
          <div className="flex gap-3">
            <button onClick={() => setActiveTab('calculator')} className="rounded-xl border border-slate-700 bg-slate-800/60 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-800 transition cursor-pointer">
              🧮 Eco Calculator
            </button>
            <button onClick={() => setActiveTab('quiz')} className="rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-emerald-500/30 hover:scale-105 transition cursor-pointer">
              ⚡ Start Economics Quiz
            </button>
          </div>
        </div>
        ...
        <div className="mt-6 pt-5 border-t border-slate-800">
          <div className="flex justify-between text-xs font-medium text-slate-300 mb-2">
            ...
            <span className="text-emerald-400 font-semibold">{xpInLevel} / {xpNeeded} XP to Level {user.level + 1}</span>
          </div>
          <div className="h-3 w-full overflow-hidden rounded-full bg-slate-800">
            <div className="h-full rounded-full bg-gradient-to-r from-emerald-400 via-cyan-500 to-indigo-500 transition-all duration-500" style={{ width: `${levelProgress}%` }}></div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 shadow-lg backdrop-blur-xl">
          ...
          <div className="text-[11px] text-slate-500">Unlocked Badges</div>
        </div>
      ...
      <div className="space-y-4">
        <h2 className="text-xl font-bold text-white flex items-center gap-2">
          <span>📊</span> Business Economics & BCK Syllabus Weightage
        </h2>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {CA_ECO_CHAPTERS.map(ch => (
            <div
              key={ch.id}
              onClick={() => setActiveTab('chapters')}
              className="cursor-pointer rounded-2xl border border-slate-800 bg-slate-900/60 p-6 shadow-lg backdrop-blur-xl transition hover:-translate-y-1 hover:border-emerald-500/40 hover:bg-slate-900/80 flex flex-col justify-between"
            >
              <div>
                <div className="flex justify-between items-center mb-4">
                  <span className="rounded-full bg-emerald-500/10 border border-emerald-500/30 px-3 py-1 text-xs font-semibold text-emerald-400">{ch.category}</span>
              ...
              <div className="flex justify-between items-center text-xs font-semibold text-emerald-400 pt-3 border-t border-slate-800/80">
                <span>{ch.formulas ? ch.formulas.length : 0} Key Concepts</span>
                <span className="flex items-center gap-1 hover:text-emerald-300">Explore Chapter →</span>
              </div>
            </div>
          ))}
        </div>
