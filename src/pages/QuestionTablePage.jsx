...
import { CA_ECO_CHAPTERS, ECO_QUESTIONS } from '../data/ecoData';
...
export function QuestionTablePage({ setActiveTab, setSelectedChapterIdForQuiz, setSelectedSubExerciseIdForQuiz }) {
  ...
  const availableSubExercises = selectedChapterId === 'All'
    ? CA_ECO_CHAPTERS.flatMap(ch => ch.subExercises || [])
    : (CA_ECO_CHAPTERS.find(ch => ch.id === selectedChapterId)?.subExercises || []);

  const filteredQuestions = ECO_QUESTIONS.filter(q => {
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white flex items-center gap-2">
            <span>📋</span> Economics Question Bank Data Table & Multi-Filters
          </h1>
          <p className="text-sm text-slate-400">
            Browse, search, and filter all CA Foundation Economics questions in a structured data view.
          </p>
        </div>

        <div className="flex items-center gap-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-2 text-xs font-bold text-emerald-400">
          🎯 Matching: {filteredQuestions.length} / {ECO_QUESTIONS.length} Questions
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 shadow-xl backdrop-blur-xl space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="md:col-span-1">
            ...
            <input
              type="text"
              placeholder="Filter by question, concept, author..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full rounded-xl bg-slate-950 border border-slate-800 px-4 py-2.5 text-xs text-white placeholder-slate-500 focus:border-emerald-500 focus:outline-none"
            />
          </div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-xl bg-slate-950 border border-slate-800 px-4 py-2.5 text-xs text-slate-200 focus:border-emerald-500 focus:outline-none"
            >
              <option value="All">All Chapters ({CA_ECO_CHAPTERS.length})</option>
            ...
            <div className="flex bg-slate-950 border border-slate-800 p-1 rounded-xl">
              {['All', 'Easy', 'Medium', 'Hard'].map(diff => (
                <button
                  ...
                  className={`flex-1 rounded-lg py-1.5 text-[11px] font-bold transition cursor-pointer ${selectedDifficulty === diff
              ...
              {filteredQuestions.length === 0 ? (
                <tr>
                  <td colSpan="5" className="py-12 text-center text-slate-500 font-medium">
                    No economics questions found matching selected filter criteria.
                  </td>
                </tr>
              ) : (
                    ...
                    <React.Fragment key={q.id || idx}>
                      <tr className="hover:bg-slate-800/40 transition">
                        ...
                        <td className="py-3.5 px-4">
                          <div className="font-bold text-emerald-400 truncate max-w-[180px]">{q.chapterName}</div>
                        ...
                        <td className="py-3.5 px-4 text-center">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <button
                              ...
                              className="rounded-lg bg-emerald-600 hover:bg-emerald-500 px-3 py-1 text-[11px] font-bold text-white shadow-sm transition cursor-pointer"
                            >
                              Practice ⚡
                            </button>
                          </div>
                      ...
                      {isExpanded && (
                        <tr className="bg-slate-950/80 border-b border-emerald-500/20">
                          <td colSpan="5" className="p-6">
                            <div className="space-y-3 rounded-xl border border-emerald-500/30 bg-slate-900/90 p-5 backdrop-blur-md">
                              <h4 className="text-xs font-bold text-emerald-400">Options:</h4>
                              ...
                              {q.formulaUsed && (
                                <div className="text-xs text-amber-300 bg-amber-500/10 border border-amber-500/30 p-2.5 rounded-lg">
                                  📐 <strong>Key Concept:</strong> <code>{q.formulaUsed}</code>
                                </div>
                              )}

                              <div className="text-xs text-slate-300 bg-slate-950 p-3 rounded-lg border border-slate-800">
                                <strong className="text-emerald-400 block mb-1">💡 Economic Explanation:</strong>
                                {q.explanation}
                              </div>
                            </div>
                          </td>
