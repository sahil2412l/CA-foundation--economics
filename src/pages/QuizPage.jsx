...
import { ECO_QUESTIONS } from '../data/ecoData';
...
export function QuizPage({ user, setUser, selectedChapterIdForQuiz, setSelectedChapterIdForQuiz, selectedSubExerciseIdForQuiz, setSelectedSubExerciseIdForQuiz }) {
  ...
  const availableQuestions = selectedSubExerciseIdForQuiz
    ? ECO_QUESTIONS.filter(q => q.subExerciseId === selectedSubExerciseIdForQuiz)
    : selectedChapterIdForQuiz
      ? ECO_QUESTIONS.filter(q => q.chapterId === selectedChapterIdForQuiz)
      : ECO_QUESTIONS;
    ...
    return (
      <div className="p-12 text-center text-white space-y-4">
        ...
        <button
          ...
          className="px-6 py-2.5 rounded-xl bg-emerald-600 text-xs font-bold text-white cursor-pointer"
        >
          Show All Questions
        </button>
      </div>
  ...
  return (
    <div className="mx-auto max-w-4xl p-8 space-y-6">
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-extrabold text-white">Interactive Economics Quiz</h1>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-8 space-y-6 shadow-xl backdrop-blur-xl">
        <div className="flex justify-between items-center flex-wrap gap-2">
          <div className="flex items-center gap-2">
            <span className="rounded-full bg-emerald-500/10 border border-emerald-500/30 px-3 py-1 text-xs text-emerald-400 font-semibold">{currentQ.chapterName}</span>
          </div>

          <div className="rounded-lg bg-emerald-500/20 border border-emerald-500/40 px-3 py-1 text-xs font-bold text-emerald-300">
        ...
        <div className="space-y-3">
          {currentQ.options.map((opt, idx) => {
            ...
            if (selectedOptionIndex === idx) btnStyle = "border-emerald-500 bg-emerald-500/20 text-white";
        ...
        {!isAnswerSubmitted ? (
          <div className="flex justify-end">
            <button
              ...
              className="rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 px-8 py-3 text-sm font-bold text-white disabled:opacity-40 shadow-lg cursor-pointer"
            >
              Submit Answer
            </button>
          </div>
          ...
          <div className="rounded-xl bg-slate-950 p-4 border border-slate-800 space-y-2">
            {currentQ.formulaUsed && (
              <div className="text-xs text-amber-300 mb-1">📐 <strong>Key Concept:</strong> <code>{currentQ.formulaUsed}</code></div>
            )}
            <div className="text-sm font-bold text-emerald-400">💡 Step-by-step Explanation</div>
